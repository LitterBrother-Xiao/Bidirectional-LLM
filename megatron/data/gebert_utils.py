...
def build_train_valid_test_datasets(train_data_prefix, valid_data_prefix, data_impl, 
                                    ...
                                    max_seq_length, has_sentence_split, masked_type,
                                    masked_x_type, has_attention_masking,
                                    seed, skip_warmup):

    # if len(data_prefix) == 1:
    return _build_train_valid_test_datasets(train_data_prefix[0], valid_data_prefix[0],
                                            data_impl, train_samples, valid_samples,
                                            max_seq_length, has_sentence_split, masked_type,
                                            masked_x_type, has_attention_masking,
                                            seed, skip_warmup)
...
def _build_train_valid_test_datasets(train_data_prefix, valid_data_prefix, data_impl,
    ...
    def build_dataset(name, indexed_dataset, data_prefix, sample_num):
        from megatron.data.gebert_dataset import GeBertDataset
        dataset = None
        kwargs = dict(
            name=name,
        ...
        dataset = GeBertDataset(
            indexed_dataset=indexed_dataset,
            encoder_seq_length=max_seq_length, 
            has_sentence_split=has_sentence_split,
            masked_type=masked_type,
            masked_x_type=masked_x_type,
            has_attention_masking=has_attention_masking,
            **kwargs
        )
        return dataset
...
def random_split(src_tokens, min_seq_length, np_rng):
    
    assert min_seq_length <= len(src_tokens)
    split_index = np_rng.randint(min_seq_length, len(src_tokens) - min_seq_length)
    source = src_tokens[:split_index]
    target = src_tokens[split_index:]
    return source, target, split_index

def bert_like_masking(tokens,
                    vocab_id_list, vocab_id_to_token_dict,
                    masked_lm_prob,
...
def span_uniform_masking(tokens,
                    vocab_id_list, vocab_id_to_token_dict,
                    masked_lm_prob,
                    eos_token_id, bos_token_id, pad_token_id, unk_token_id, mask_token_id,
                    max_predictions_per_seq,
    ...
    Note: Tokens here are vocab ids and not text tokens."""
    if max_ngrams==0:
        max_ngrams=max_predictions_per_seq
    ...
    ngrams = np.arange(min_ngram, max_ngrams + 1, dtype=np.int64)
    if not geometric_dist:
        # Note(mingdachen):
        # By default, we set the probilities to favor shorter ngram sequences.
        pvals = 1. / np.arange(1, max_ngrams + 1)
...
def lazy_uniform_masking(tokenized_ids_target, mask_token_id, np_rng):
    # import pdb
    # pdb.set_trace()
    # tokenized_ids_target = tokenized_ids_target.tolist()
    seq_len = len(tokenized_ids_target)
    mask_choices = np.arange(1, seq_len)
    num_subwords_to_mask = int((0.3 + np_rng.rand() * 0.4) * seq_len)
    masked_positions = sorted(np.random.choice(mask_choices,
                                  num_subwords_to_mask,
                                  replace=False))
    labels = []
    for index in masked_positions:
        labels.append(tokenized_ids_target[index])
        tokenized_ids_target[index] = mask_token_id

    return tokenized_ids_target, masked_positions, labels