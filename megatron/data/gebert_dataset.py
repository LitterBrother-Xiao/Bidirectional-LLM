...
def build_training_sample(src_tokens, tgt_tokens, encoder_seq_length,
    ...
    split_list = np.argwhere(src_tokens == 65535).tolist()
    split_list = [index[0] for index in split_list]
    src_tokens = src_tokens.tolist()
    assert len(split_list) == 4
    merge_tokens_all = src_tokens[:split_list[0]]
    merge_labels_all = src_tokens[split_list[0]+1:split_list[1]]
    merge_masked_positions_all = src_tokens[split_list[1]+1:split_list[2]]
    source_attention_index_list = src_tokens[split_list[2]+1:split_list[3]]
    target_attention_index_list = src_tokens[split_list[3]+1:]
...
#                 src_tokens = src_tokens.tolist()
#                 mask_lm_prob = np_rng.rand()
#                 max_predictions_per_seq = mask_lm_prob * len(src_tokens)
...
#                         mask_lm_prob = 0.3 + np_rng.rand() * 0.4
#                         max_predictions_per_seq = mask_lm_prob * len(single_sample)
#                         masked_tokens, masked_positions, masked_labels, _, _ = span_uniform_masking(
...
#                 # xys76
#                 target_max_predictions_per_seq = target_masked_lm_prob * len(single_tgt_tokens)
...
#                     #xys76
#                     source_masked_lm_prob = 0.1 + 0.2 * target_masked_lm_prob
...
#                 tokens_np, labels_np, padding_mask_np, loss_mask_np, index_mask_np \
#                     = pad_and_convert_to_numpy_group(merge_tokens, merge_masked_positions,
#                                             merge_labels, pad_token_id, encoder_seq_length, [attention_index], [encoder_seq_length])      
