...
class _SentencePieceTokenizer(AbstractTokenizer):
    ...
    def _initalize(self, vocab_extra_ids):
        ...
        self._t5_sent_tokens = ['<pad>', '</s>', '<unk>', '<bos>', '<mask>']
        self._t5_mask_tokens = [f'<extra_id_{i}>' for i in range(vocab_extra_ids - 1, -1, -1)]

        self._t5_tokens = self._t5_sent_tokens + self._t5_mask_tokens
...
class _HFTokenizer(AbstractTokenizer):
    ...
    def __init__(self, tokenizer_name_or_path):
        ...
        self._mask_token_id = self.vocab.get(self.mask_token)
        # import pdb; pdb.set_trace()
        self.bos_token = '[BOS]'
        self.add_token(self.bos_token)
        self._bos_token_id = self.vocab.get(self.bos_token)

        self.pad_token = '[PAD]'
        self.add_token(self.pad_token)
        self._pad_token_id = self.vocab.get(self.pad_token)
        # import pdb; pdb.set_trace()
        # vocab_extra_ids=10
        # additional_special_tokens = []
        # additional_special_tokens.extend(
    ...
    def add_token(self, token):
        if token not in self.encoder:
            ...
            self.decoder[new_token_id] = token
            # self.vocab_size comes from len(vocab)
            # and it will increase as we add elements
            self.encoder[token] = new_token_id
